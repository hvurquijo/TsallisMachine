...
program Tsallis_graphs
  ...
  integer :: k, i, ii, n, m, cycl, iii, fi              !n is the number of spins, m is the size o the interval for variating q
  ...
  interface
     ...
     subroutine TS_mean_calculator(h,J,q,q_exp,beta,n,si_mean,sij_mean,sijk_mean,err_J,err_h,&
...
!-------------------------FIN_CORREÇÃO_ENERGIA---------------------------------------
  ...
  m=11
  allocate(q_mat(m))
  q_mat = (/0.70,0.75,0.80,0.85,0.90,0.95,1.00,1.05,1.10,1.15,1.20/)
  allocate(beta_mat(1))
  beta_mat = (/0.13/)
  print*,q_mat
  ...
  !$OMP PARALLEL 
      !$OMP DO
         do i=1,m
            call TS_mean_calculator(H,J,q_mat(i),q,BETA,n,si_mean(i),sij_mean(i),&
...
subroutine TS_mean_calculator(h,J,q,q_exp,beta,n,si_mean,sij_mean,sijk_mean,err_J,err_h,&
 ...
 double precision :: tmp, tmp1, Zq_exp, Zq, prob, err, factor, sumdev !temporal variables and partition functions
 integer :: i,ii, k,kk,kkk, counter !for "do" cycles
