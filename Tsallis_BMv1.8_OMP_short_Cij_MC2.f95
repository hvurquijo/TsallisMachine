...
!-------------------------FIN_CORREÇÃO_ENERGIA---------------------------------------
  ...
  q_mat = (/0.80,0.79,0.81,0.78,0.82,0.77,0.83,0.76,0.84,0.75,0.85/)
  allocate(beta_mat(1))
  beta_mat = (/0.02/)
  print*, q_mat
...
subroutine TS_mean_calculator(ThrNumber,h,J,q,q_exp,beta,n,samples,si_mean,sij_mean,sijk_mean,err_J,err_h,&
 ...
 double precision :: tmp, Zq_exp, Zq, prob !temporal variables and partition functions
 double precision :: isingtmp1, isingtmp2, e1,e2, err, factor, sumdev, tmpdbl !temporal variables and partition functions
 integer :: i, ii, iii, k, kk, kkk !for "do" cycles
...
yesno='y'
 ...
 !factor = 2
print*,"Chegou aqui, antes do calculate experimental mean"
allocate(Zq_exp_mat(ns), states(n,ns), Zq_mat(ns))
 !---------------------------------------------------------------------
 ...
 factor = 1
 !---------------------------------------------------------------------
 print*,"Definuiu os valores mas ainda não entrou no calculate experimental mean"
...
print*,"Calculou função de partição"
 do i=0,ns-1     
   if(Q_exp==1.0)then                               
 ...
 print*,"As medias experimentais são: "
 print*, mean_si_exp
 !-----------------------------------------------------------------------------------------
 ...
 label1:do while  (abs(si_mean+sij_mean)/2>err) !this "do" controls the number of aproximations the time will make
   ...
   !$OMP DO
      do iii=1,ThrNumber
         ...
         global_mean_si_TS(:,iii) = global_mean_si_TS(:,iii)/counter(iii)
         global_mean_sij_TS(:,:,iii) = global_mean_sij_TS(:,:,iii)/counter(iii)
         global_mean_sijk_TS(:,:,:,iii) = global_mean_sijk_TS(:,:,:,iii)/counter(iii)
      enddo
   !$OMP END DO 
   !$OMP END PARALLEL   
   ...
   !-----------------------------------------------------------------------------------------------------
    ...
    enddo
    !------------------------------------------------------------------------------------
    if(mod(counter(1)-1,3)==0 .OR. counter(1) == 1)then
      tmpdev(1)=abs(si_mean+sij_mean)/2
    elseif(mod(counter(1)-1,2)==0)then
      tmpdev(2)=abs(si_mean+sij_mean)/2
    endif
    sumdev=sumdev+tmpdev(1)*tmpdev(2)

    if(mod(counter(1),250)==0)then !for debuggin
       print *, " "
       ...
       print*, mean_si_TS
       print *, " "
       if(abs(si_mean+sij_mean)/2>err)then
         print*,"Do you want to change the value of factor? It is currently: ",factor
         print*," "
         read*,yesno
         if (yesno=='y'.or.yesno=='Y')then
            print*,"Write the value of factor: "
            read*,factor
         endif
      endif
